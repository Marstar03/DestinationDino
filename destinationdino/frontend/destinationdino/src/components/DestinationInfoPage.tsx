...
const DestinationInfoPage: React.FC = () => {
  ...
  const apiUrl = `http://localhost:8080/destinationInfo?id=${encodeURIComponent(name as string)}`;
  ...
  return (
    <div>
      ...
      <GridContainer>
        ...
        <Heading>
          <Title>
            {data && <BoxForDestinationInfo title="Destination" content={`${data.name}, ${data.country}`} />}
          </Title>
          <Score>
            <Rating>
        ...
        <Description>
          {data && <BoxForDestinationInfo title="Short Description" content={data.info} />}
        </Description>
        <Attraction>
          {data && <BoxForDestinationInfo title="Review" content="Review Function Coming Soon" />}
        </Attraction>
      </GridContainer>
    </div>
  );
